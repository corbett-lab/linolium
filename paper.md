...
The Linolium lineage curation browser interface is built with the Taxonium library. We extended Taxonium with lineage-aware visualization components that directly ingest AutoLin proposals (\autoref{Figure 1}), enable node-level lineage reassignment (\autoref{Figure 2} A and B), and export updated lineage annotations in a format compatible with downstream phylogenetic analysis (\autoref{Figure 1}). This creates a unified environment in which automated lineage inference and expert-driven revision operate on the same underlying tree representation. This design supports iterative cycles of automated lineage proposal followed by expert curation within the same analytical environment.  By centralizing these tools into a unified, containerized application, Linolium consolidates previously separate command-line workflows into a single reproducible system for lineage designation and curation.

![Visualization of M. tuberculosis Lineage 4.8 phylogeny with pre-existing and AutoLin assigned lineages within Linolium browser interface. A) The mutation annotated trees and edited lineage information can be exported directly from the browser in JSONL format or edited lineage information can be exported in tsv format. B) All lineages within the MAT can be viewed and searched and descending lineages can be collapsed within viewer. C) Full interactive tree displayed, with ability to zoom on specific clades and nodes.\label{Figure 1}](Figure1.svg){width=100%}

![A) Linolium browser in lineage reassignment node for AutoLin designated lineage 4.8.62. In this mode any node can be selected as the new parent node for the lineage. B) AutoLin designated Lineage auto.lineage4.8.62 after parent node reassignment. Node reassignment will identify all descendent leaves and assign their correct lineage. In addition to parent node reassignment, a lineage can also be merged into the parent lineage, essentially rejecting a suggested lineage.\label{Figure 2}](Figure2.svg){width=80%}
